...
theta=1.5 %1.25-2.25 Leisure in utility function. Otherwise makes MU leisure non-decreasing in leisure
sdzstep=.2; %.005
sdzsteps=1
...
A=((1/beta-1+delta)/(alpha*(1-alpha)^((1-alpha)/(theta+alpha-1))*kssTh^((1-alpha)*(1-theta)/(theta+alpha-1))))^((theta+alpha-1)/theta);
nssTh=((1-alpha)*A*kssTh^alpha)^(1/(theta+alpha-1));
...
gpz=19; % 19,21 odd number>=5 Stops running properly if too large, and optimal k-rule goes out of bounds with so high shock values

T=1e5 % 1e8 (if HP filtering) or 2e8, simulation length. 4e8 is too much for 16GB RAM
Reps=1e1/5 % 1e3 Repetitions in simulations. Faster with larger T, less reps.
...
parfor ix = 1:numberiterations
    ...
    % GRiskGHHRCNfocfun, GRiskGHHRCNfocnoCACfun                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             GRiskGHHRCNfocfun, GRiskGHHRCNfocnoCACfun
    [S_KTILDEZ0n(:,ix),S_NOPTZ0n(:,ix),S_CZ0n(:,ix),S_VZ0n(:,ix),S_KMAXn(:,ix),S_KBARn(:,ix),S_KSDn(:,ix),S_KSSMn(:,ix),S_KSSNOn(:,ix),S_NBARn(:,ix),S_NSDn(:,ix),S_CBARn(:,ix),S_CSDn(:,ix),S_IBARn(:,ix),S_ISDn(:,ix),S_YBARn(:,ix),S_YSDn(:,ix),S_V1n(:,ix),S_V2n(:,ix),S_V3n(:,ix),S_V4n(:,ix),S_ITERn(:,ix),S_TRASHITn(:,ix),S_TRASHMAXn(:,ix),Vn(:,:,ix),Ktilden(:,:,ix),Nn(:,:,ix),Cn(:,:,ix),Zn(:,ix),S_KMINn(:,ix),S_CSDCOMPDATAn(:,ix),S_CACCOMPDATAn(:,ix),S_CACn(:,ix),S_NSDCOMPDATAn(:,ix),S_NACCOMPDATAn(:,ix),S_SIGMAn(:,ix),S_SDZn(:,ix)] = GRiskGHHRCNfocnoCACfun(sdz,sigma,A,alpha,rho,beta,mu,delta,theta,gp,gpz,T,Ti,itermax,k,v,UsekssTh,kssTh,countsdz,Reps);
end
...
clear S_KTILDEZ0n S_NOPTZ0n S_CZ0n S_VZ0n S_KMAXn S_KBARn S_KSDn S_KSSMn S_KSSNOn S_NBARn S_NSDn S_CBARn S_CSDn S_IBARn S_ISDn S_YBARn S_YSDn S_V1n S_V2n S_V3n S_V4n S_ITERn S_TRASHITn S_TRASHMAXn Vn Ktilden Nn Cn Zn S_KMINn S_CSDCOMPDATAn S_CACCOMPDATAn S_CACn S_NSDCOMPDATAn S_NACCOMPDATAn S_SDZn S_SIGMAn

tv = datetime('now', 'Format','yyyy_MM_dd_HH_mm');
filename = sprintf('GRiskGHHRCNfocPC_%s.mat',tv);
...
% end
% axis([-1 1.45*kssTh -1 1.45*kssTh]);
% title('Optimal capital rule, Z=E(Z)','FontSize',12);
% xlabel('\it k_t','FontSize',12);
% ylabel('\it k_{t+1}','FontSize',12,'rotation',0);
% centeraxes(gca);
...
for i=1:lsigmavect
    for j=1:lsdzvect
        cssGainPercentVkss(i,j)=((((1-beta)*(1-sigmavect(i))*(S_V1(1,j,i)-S_V1(1,lsdzvect,i))+(cssTh-nssTh^theta/theta)^(1-sigmavect(i)))^(1/(1-sigmavect(i)))+nssTh^theta/theta)/cssTh-1)*100;
    end
end
for i=1:lsigmavect
...
for i=1:lsigmavect
        cssGainPercentVk(:,i)=((((1-beta)*(1-sigmavect(i))*(dVZ0(:,i))+(cssTh-nssTh^theta/theta).^(1-sigmavect(i))).^(1/(1-sigmavect(i)))+nssTh^theta/theta)/cssTh-1)*100;
end
for i=1:lsigmavect
...
for i=1:lsigmavect
    for j=1:lsdzvect
        cssGainPercentVkbar(i,j)=((((1-beta)*(1-sigmavect(i))*(S_V3(1,j,i)-S_V3(1,lsdzvect,i))+(cssTh-nssTh^theta/theta)^(1-sigmavect(i)))^(1/(1-sigmavect(i)))+nssTh^theta/theta)/cssTh-1)*100;
    end
end
for i=1:lsigmavect
