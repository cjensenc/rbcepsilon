...
% Level of A affects value function and welfare relative to without
% uncertainty, if welfare is close to zero (c=0 or c=1 depending on -1 in
% u-function).

% sd(c) in HP-filetered data is 1.21, .82-1.78 by decades
% First-order ac c in HP-filtered data is .69
% HP-filtered AC too low for (rho=.95):
% sigma=0, sdz=.25 (A=1; .01; 100)

% Non-filtered ac c too high with rho=.95. Many too low when lower enough
% for sigma=4 (rho=0). Focus on filtered ac, is more stable

%digits(32); % 16 controls precision and tolerance for convergence
alpha=1/3 %1/3;
rho=.95 %.9 CALIBRATE WITH FILTERED
beta=.99;%.989-.995
mu=0; %safe income
delta=.025; %.02 also yields safe income .01-.04 quarterly
sdzsteps=1 %20
sdzstep=.2/sdzsteps; %.01
sigmavect=[4; 3; 2; 1.01; .5];% CHANGE FIG LABELS IF CHANGE, [4, 3, 2, 1.01, .5, 0] % sigma can't equal 1
...
% sigmavect=[2; 1.75; 1.5; 1.25; 1.01; .75; .5; .25; 0];% CHANGE FIG LABELS IF CHANGE, [4, 3, 2, 1.01, .5, 0] % sigma can't equal 1
% labels={[char(949) '=2']; [char(949) '=1.75']; [char(949) '=1.5']; [char(949) '=1.25']; [char(949) '=1.01']; [char(949) '=.75']; [char(949) '=.5']; [char(949) '=.25']; [char(949) '=0']};
% colors={[0.9047 0.1918 0.1988]; [0.2941 0.5447 0.7494]; [0.3718 0.7176 0.3612]; [1.0000 0.5482 0.1000]; [0.8650 0.8110 0.4330]; [0.6859 0.4035 0.2412]; [0.9718 0.5553 0.7741]; [0.6400 0.6400 0.6400]; [0.6365 0.3753 0.6753]}; % colors generated by linspecer.m

%65 32
gp=1000 %8000; % 4000 even number of gridpoints k-vector
...
T=1e8/100; % 1e8 (if HP filtering) or 2e8, simulation length. 4e8 is too much for 16GB RAM
...
% Fineness of grid, also k3, matter for results?
% gp=1000, 16%, .14, .7
% gp=2000, 8%, 1533, .7667 
% gp=3000, 
% gp=4000, 4%, .16, .8
...
% kmaxgp=kssTh*10 % 10 (sdz=.2); 24 (sdz=.25)
% %k=linspace(0,kmaxgp,gp)'; %makes k be a discrete grid
% k1=linspace(0,kssTh,.164*gp+1)';
% k2=linspace(kssTh+k1(2),kssTh*6,.82*gp)'; %6 in here Adjust so kmax from sim is within k2 (fine grid)
