function [ktildez0,noptz0,cz0,vz0,kmax,kbar,ksd,kssM,kssno,nbar,nsd,cbar,csd,ibar,isd,ybar,ysd,v1,v2,v3,v4,iter,trashit,trashmax,v,ktildeposition,ktilde,n,c,z,kmin,sigma,sdz,psi] = GRiskADSRCNfocfun(sdz,sigma,theta,psi,A,alpha,rho,beta,mu,delta,gp,gpz,T,Ti,itermax,k,v,UsekssTh,kssTh,nssTh,Reps)
...
for i=1:gp
    for j=1:gp
        for m=1:gpz
            ...
            if sigma==0
                nopt = ((1-alpha)*A*(1+z(m))*(k(i))^alpha/psi)^(1/(alpha+theta));
                noptcondj(i,j,m)=nopt;
            else
                solnsqrt = fzero( @(nvar) (A*(1+z(m))*(k(i))^alpha*(real(nvar^2))^(1-alpha)+(1-delta)*k(i)-k(j)-(((1-alpha)*A*(1+z(m))*(k(i))^alpha*(real(nvar^2))^(-alpha-theta))/psi)^(1/sigma)), sqrt(nssTh)*0+.01, options);
                nopt = solnsqrt^2; % Square in function to only get positive solutions
                noptcondj(i,j,m)=nopt;
            end


            if nopt >= 0 && (1+z(m))*A*(k(i))^alpha*nopt^(1-alpha)+(1-delta)*k(i)+mu-k(j) >= 0
                U0(i,j,m)=((1+z(m))*A*(k(i))^alpha*nopt^(1-alpha)+(1-delta)*k(i)+mu-k(j))^(1-sigma)/(1-sigma)-psi*nopt^(1+theta)/(1+theta);
            else
                U0(i,j,m)=-inf;
            end
